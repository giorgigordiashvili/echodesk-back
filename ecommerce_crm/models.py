...
class ItemList(models.Model):
    """
    Product lists/collections for ecommerce storefront
    Examples: "Featured Products", "New Arrivals", "Best Sellers", "Summer Collection"
    Merchants create these in settings to organize and display products
    """
    title = models.JSONField(
        help_text="List title in different languages: {'en': 'Featured Products', 'ka': 'გამორჩეული პროდუქტები'}"
    )
    description = models.JSONField(
        blank=True,
        default=dict,
        help_text="List description in different languages"
    )
    slug = models.SlugField(
        max_length=200,
        help_text="URL-friendly identifier for this list"
    )
    is_public = models.BooleanField(
        default=True,
        help_text="Whether this list is visible to ecommerce clients"
    )
    is_active = models.BooleanField(
        default=True,
    ...
    created_by = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        on_delete=models.CASCADE,
        related_name='created_ecommerce_item_lists'
    )
    ...
    def __str__(self):
        if isinstance(self.title, dict):
            return self.title.get('en', self.title.get(list(self.title.keys())[0], self.slug))
        return str(self.title)

    def get_title(self, language='en'):
        """Get list title in specific language"""
        if isinstance(self.title, dict):
            return self.title.get(language, self.title.get('en', self.slug))
        return str(self.title)

    def get_description(self, language='en'):
        """Get list description in specific language"""
        if isinstance(self.description, dict):
            return self.description.get(language, self.description.get('en', ''))
        return str(self.description)


class ItemListProduct(models.Model):
    """
    Many-to-many relationship between ItemLists and Products
    Allows products to be in multiple lists with custom positioning
    """
    ...
    position = models.PositiveIntegerField(
        default=0,
        help_text="Position of product in this list"
    )
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=['item_list', 'position']),
            models.Index(fields=['product']),
        ]

    def __str__(self):
        return f"{self.item_list} - {self.product.sku}"
