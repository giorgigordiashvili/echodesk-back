...
# =============================================================================
# SOCIAL SYNC ENDPOINTS
# =============================================================================

@api_view(['GET'])
@permission_classes([IsAuthenticated, CanManageSocialConnections])
def social_sync_status(request, platform=None):
    """
    Get sync status for Facebook pages and/or Instagram accounts.

    GET /api/social/sync/status/           - Get status for all platforms
    GET /api/social/sync/status/facebook/  - Get status for Facebook only
    GET /api/social/sync/status/instagram/ - Get status for Instagram only
    """
    result = {}
            ...
            'pages': [
                {
                    ...
                    'last_sync_error': page.last_sync_error or None,
                    'sync_days_back': page.sync_days_back,
                    'conversations_synced': page.conversations_synced,
                    'messages_synced': page.messages_synced,
                }
                for page in pages
            ],
            'total_pages': pages.count(),
            'pending_count': pages.filter(sync_status='pending').count(),
            'syncing_count': pages.filter(sync_status='syncing').count(),
            'completed_count': pages.filter(sync_status='completed').count(),
            'failed_count': pages.filter(sync_status='failed').count(),
        }
            ...
            'accounts': [
                {
                    ...
                    'last_sync_error': account.last_sync_error or None,
                    'sync_days_back': account.sync_days_back,
                    'conversations_synced': account.conversations_synced,
                    'messages_synced': account.messages_synced,
                }
                for account in accounts
            ],
            'total_accounts': accounts.count(),
            'pending_count': accounts.filter(sync_status='pending').count(),
            'syncing_count': accounts.filter(sync_status='syncing').count(),
            'completed_count': accounts.filter(sync_status='completed').count(),
            'failed_count': accounts.filter(sync_status='failed').count(),
        }
...
@api_view(['POST'])
@permission_classes([IsAuthenticated, CanManageSocialConnections])
def social_sync_trigger(request):
    """
    Trigger manual sync for Facebook pages and/or Instagram accounts.

    POST /api/social/sync/trigger/

    Request body:
    {
        "platform": "facebook" | "instagram" | "all",  // Default: "all"
        "account_id": "123456789",  // Optional: specific account
        "force": false  // Optional: force resync even if completed
    }
    """
    from .facebook_sync_utils import (
    ...
    if platform in ['facebook', 'all']:
        ...
        for page in pages:
            try:
                ...
                page.save(update_fields=['sync_status'])
                ...
                logger.error(f"Error syncing Facebook page {page.page_name}: {e}")
                page.sync_status = 'failed'
                page.last_sync_error = str(e)
                page.save(update_fields=['sync_status', 'last_sync_error'])
    ...
    if platform in ['instagram', 'all']:
        ...
        for account in accounts:
            try:
                ...
                logger.error(f"Error syncing Instagram @{account.username}: {e}")
                account.sync_status = 'failed'
...
@api_view(['POST'])
@permission_classes([IsAuthenticated, CanManageSocialConnections])
def social_sync_settings(request):
    """
    Update sync settings for a Facebook page or Instagram account.

    POST /api/social/sync/settings/

    Request body:
    {
        "platform": "facebook" | "instagram",
        "account_id": "123456789",
        "sync_days_back": 30  // Optional: days of history to sync (1-365)
    }
    """
    platform = request.data.get('platform')
    account_id = request.data.get('account_id')
    ...
    # Validate sync_days_back
    if sync_days_back is not None:
        try:
            sync_days_back = int(sync_days_back)
            if sync_days_back < 1 or sync_days_back > 365:
                return Response(
                    {'error': 'sync_days_back must be between 1 and 365'},
                    status=status.HTTP_400_BAD_REQUEST
                )
    ...
    try:
        if platform == 'facebook':
            connection = FacebookPageConnection.objects.get(page_id=account_id)
            if sync_days_back is not None:
                connection.sync_days_back = sync_days_back
                connection.save(update_fields=['sync_days_back'])
            ...
            return Response({
                'platform': 'facebook',
                'account_id': connection.page_id,
                'account_name': connection.page_name,
                'sync_days_back': connection.sync_days_back,
            })

        else:  # instagram
            connection = InstagramAccountConnection.objects.get(instagram_account_id=account_id)
            if sync_days_back is not None:
                connection.sync_days_back = sync_days_back
                connection.save(update_fields=['sync_days_back'])
            ...
            return Response({
                ...
                'account_name': f'@{connection.username}',
                'sync_days_back': connection.sync_days_back,
            })

    except FacebookPageConnection.DoesNotExist:
        return Response(
            {'error': f'Facebook page {account_id} not found'},
            status=status.HTTP_404_NOT_FOUND
        )
    except InstagramAccountConnection.DoesNotExist:
        return Response(
            {'error': f'Instagram account {account_id} not found'},
            status=status.HTTP_404_NOT_FOUND
        )